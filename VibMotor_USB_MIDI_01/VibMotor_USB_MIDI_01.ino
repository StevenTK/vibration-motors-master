...
void setup() {
  Serial.begin(115200);
  pinMode( motor1 , OUTPUT); //Vibration Motor
}
...
void playNote(byte noteNum, byte velocity) {
  ...
  int velScaled = map(velocity, 0, 127, 0, 153);

  if (noteNum == 60) {
    analogWrite(motor1,velScaled);
  }
...
void controlChange (byte ccNum, byte val) {
  ...
  int valScaled = map(val, 0, 127, 0, 153);

  if (ccNum == 60) {
    analogWrite(motor1,valScaled);
  }
